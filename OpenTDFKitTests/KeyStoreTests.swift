...
final class PublicKeyStoreTests: XCTestCase {
    func testPublicKeyStoreExportAndConsumption() async throws {
        ...
        let publicKey = try await receivedStore.getAndRemovePublicKey()
    ...
    func testP2PEncryptionDecryption() async throws {
        ...
        guard let privateKey = try await peerAKeyStore.getPrivateKey(forPublicKey: kasMetadata.getPublicKey()) else {
        ...
        let cryptoHelper = CryptoHelper()
        let ephemeralKeyPair = try EphemeralKeyPair(
            privateKey: privateKey,
            publicKey: kasMetadata.getPublicKey(),
            curve: .secp256r1
        )
